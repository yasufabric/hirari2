export const LANE_COUNT = 3

export const PLAYER_ABOVE_PADS = 48
export const PLAYER_RADIUS = 24
export const PLAYER_EASE = 14 // higher = snappier follow toward the target lane
export const PLAYER_DRAW_SCALE_CAP = 0.7
export const PLAYER_BASE_REACH = 41
export const MAX_MUSCLE_LEVEL = 8

export const OBSTACLE_SIZE = 34
export const PROTEIN_SIZE = 30
export const PROTEIN_SCORE = 80
export const PROTEIN_COMBO_BONUS = 25
export const PROTEIN_SPAWN_CHANCE = 0.42
export const HIRARI_SCORE = 20
export const MUSCLE_SCORE_STEP = 0.125
export const BASE_OBSTACLE_SPEED = 240 // px/s
export const OBSTACLE_SPEED_GROWTH = 6 // px/s of speed added per second survived

export const BASE_SPAWN_INTERVAL_MS = 900
export const MIN_SPAWN_INTERVAL_MS = 380
export const SPAWN_INTERVAL_DECAY_PER_SEC = 12

export const SCORE_PER_SECOND = 6

export const BEST_SCORE_STORAGE_KEY = 'hirari2.bestScore'
export const MUTE_STORAGE_KEY = 'hirari2.muted'
